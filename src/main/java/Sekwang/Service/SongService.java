...
public class SongService {
    ...
    public Song create(SongDto.CreateReq req){
        ...
        Song.Category cat;
        try { cat = req.category()==null? Song.Category.기타 : Song.Category.valueOf(req.category()); }
        catch (Exception e){ throw new IllegalArgumentException("category 값이 올바르지 않습니다."); }

        Song s = Song.builder()
                .uploader(uploader).title(req.title()).artist(req.artist())
                .imageUrl(req.imageUrl()).category(cat)
                .musicalKey(req.musicalKey()).tempoBpm(req.tempoBpm())
                .build();
        return repo.save(s);
    }
    ...
    public Page<Song> list(String category, int page, int size){
        ...
        Song.Category cat = Song.Category.valueOf(category);
