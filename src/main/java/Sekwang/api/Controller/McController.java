...
public class McController {
    ...
    // ✅ 추가: 세트 전체 목록 조회
    @GetMapping("/sets")
    public List<McDto.SetRes> getAllSets() {
        return svc.listAllSets().stream()
                .map(s -> new McDto.SetRes(
                        s.getSetId(),
                        s.getSetName(),
    ...
    public McDto.SetWithQuestions getSet(@PathVariable Long setId) {
        McQuizSet s = svc.getSetById(setId);
        List<McQuestion> qs = svc.listQuestions(setId);
        var setRes = new McDto.SetRes(setId, s.getSetName(), null, null);
        var qRes = qs.stream()
                .map(q -> new McDto.QRes(q.getId(), setId, q.getQuestion(),
                        q.getChoice1(), q.getChoice2(), q.getChoice3(), q.getChoice4(), q.getAnswerNo()))
                .toList();
    ...
    public McDto.ResultRes submit(@RequestBody @Valid McDto.SubmitReq req) {
        ...
        return new McDto.ResultRes(r.getId(), r.getUser().getUsername(),
    ...
    public List<McDto.ResultRes> leaderboard(@PathVariable Long setId) {
        return svc.leaderboard(setId).stream()
                .map(r -> new McDto.ResultRes(r.getId(), r.getUser().getUsername(),
                        r.getSet().getSetId(), r.getScore(), r.getTakenAt().toString()))
                .toList();
    }
}
